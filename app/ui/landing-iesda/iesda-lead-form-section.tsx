...
export default function IesdaLeadFormSection() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("iesda:preselect-programa", handler);
  }, []);

  const { data: programas = [] } = useProgramasDestacadosQuery({ limit: 20, instituto: 2 });
  ...
  const inputBorder = "border-white/20 focus:border-[#D7A22A] focus:ring-[#D7A22A]/25";
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(145deg, #1C1208 0%, #2A1D0A 100%)" }}
    >
      {/* Textura de fondo */}
        ...
        <div ref={sectionRef} className="grid lg:grid-cols-[1fr_1.4fr] gap-12 lg:gap-20 items-start">
          ...
          >
            ...
            <div className="mb-6 w-12 h-12 flex items-center justify-center rounded-2xl"
              style={{ background: "rgba(215,162,42,0.12)", border: "1px solid rgba(215,162,42,0.3)" }}>
              <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="#D7A22A" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
              </svg>
            </div>

            <span className="font-medium text-sm uppercase tracking-wider" style={{ color: GOLD }}>
            ...
            <p className="mt-4 text-lg leading-relaxed" style={{ color: "rgba(255,255,255,0.65)" }}>
              ...
              programa que mejor se ajuste a tu perfil, objetivos y área de interés.
            </p>
              ...
              {TRUST_ITEMS.map((item) => (
                <li key={item} className="flex items-center gap-3 text-sm" style={{ color: "rgba(255,255,255,0.7)" }}>
                  <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center"
                    style={{ background: "rgba(215,162,42,0.18)", border: "1px solid rgba(215,162,42,0.4)" }}>
                    <svg className="w-3 h-3" fill="none" stroke="#D7A22A" viewBox="0 0 24 24">
            ...
            <div className="mt-10 p-5 rounded-2xl"
              style={{ background: "rgba(215,162,42,0.07)", border: "1px solid rgba(215,162,42,0.2)" }}>
              <p className="text-xs font-medium uppercase tracking-widest mb-2" style={{ color: GOLD }}>
                Lema de IESDA
              </p>
            ...
            <div className="mt-6 flex items-center gap-4 p-4 rounded-2xl"
              style={{ background: "rgba(215,162,42,0.06)", border: "1px solid rgba(215,162,42,0.15)" }}>
              <div className="flex -space-x-2">
                {["#D7A22A", "#C4943A", "#2A2118", "#7D8EA3"].map((color, i) => (
                  <div key={i}
          ...
          >
            <div
              ...
              style={{
                ...
                ...(highlighted ? { "--tw-ring-color": "rgba(215,162,42,0.4)" } as React.CSSProperties : {}),
              }}
            >
              ...
              {highlighted && !successMsg && (
                <motion.div
                  ...
                  style={{ background: "rgba(215,162,42,0.12)", border: "1px solid rgba(215,162,42,0.35)" }}
                >
                  ...
                  <p className="text-sm font-medium leading-snug" style={{ color: "#D7A22A" }}>
                    Programa seleccionado.{" "}
                    <span className="font-normal" style={{ color: "rgba(255,255,255,0.55)" }}>
              ...
              {successMsg ? (
                <div className="flex flex-col items-center text-center py-8 gap-4">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center"
                    style={{ background: "rgba(215,162,42,0.15)", border: "1px solid rgba(215,162,42,0.4)" }}>
                    <svg className="w-8 h-8" fill="none" stroke="#D7A22A" viewBox="0 0 24 24">
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-5">
                  ...
                  {/* Programa */}
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: "rgba(255,255,255,0.75)" }}>
                      Programa de interés *
                    </label>
                    <select
                      name="programa_ref"
                    ...
                    >
                      <option value="" style={{ background: "#2A2118" }}>Selecciona un programa…</option>
                      {programasConRef.map((p) => (
                        <option key={p.ref} value={p.ref} style={{ background: "#2A2118" }}>
                          {p.nombre}
                        </option>
                      ))}
                    </select>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: "rgba(255,255,255,0.75)" }}>
                      Apellido materno{" "}
                      <span className="font-normal" style={{ color: "rgba(255,255,255,0.35)" }}>(opcional)</span>
                    </label>
                    <input
                  ...
                  <p className="text-center text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>
                    Al enviar aceptas que un asesor de IESDA te contacte. Sin spam, sin compromisos.
                  </p>
                </form>
              )}
            </div>
