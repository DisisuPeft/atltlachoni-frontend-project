...
function solicitarPrograma(ref: string | undefined) {
  if (ref) {
    window.dispatchEvent(
      new CustomEvent("thales:preselect-programa", { detail: ref }),
    );
  }
  document
    .getElementById("solicitar-informacion-thales")
    ?.scrollIntoView({ behavior: "smooth" });
}

export default function OfertaEducativaPage() {
  const { data: programas = [], isLoading } = useProgramasDestacadosQuery({
    limit: 8,
    instituto: 1,
  });
            ...
            {programas.map((programa, index) => (
              ...
              >
                ...
                >
                  {programa.imagen_url ? (
                    ...
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, #1E3A5F 0%, #1FBAC4 100%)",
                      }}
                    >
                ...
                <div className="p-5 sm:p-6 flex flex-col flex-grow">
                  ...
                  <h3
                    className="text-lg font-semibold mb-3 flex-1 leading-snug"
                    style={{ color: "#1E3A5F" }}
                  >
                    {programa.nombre}
                  </h3>
