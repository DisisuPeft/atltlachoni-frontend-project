...
// ── PDF viewer — usa preview_url del backend directamente ─────────────────
...
function PdfViewer({ material }: { material: Material }) {
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden border border-gray-200 flex flex-col">
      ...
      <iframe src={previewUrl} className="w-full" style={{ height: "70vh" }} title={material.original_name} />
    </div>
  );
}
...
function MaterialViewer({ material }: { material: Material }) {
  ...
  if (material.file_type === "image") {
    ...
    return <img src={streamUrl} alt={material.original_name}
  ...
  return (
    <div className="w-full bg-gray-50 border border-gray-200 rounded-xl p-8 flex flex-col items-center gap-4">
      ...
      <p className="text-sm font-medium text-gray-700 text-center">{material.original_name}</p>
      <a href={streamUrl} target="_blank" rel="noopener noreferrer"
        className="inline-flex items-center gap-2 px-4 py-2 bg-[#0056D2] text-white text-sm font-medium rounded-lg hover:bg-[#004BB5] transition-colors">
        <Download className="w-4 h-4" />
        Descargar archivo
...
// ── Module accordion — fetches its own materials ───────────────────────────
...
function ModuloAccordion({
  ...
  const { data: materialesData, isLoading } = useGetMaterialesModuloQuery(modulo.id, {
    skip: !isOpen,
  });
  const archivos = useMemo(() => materialesData?.results ?? [], [materialesData]);
    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden">
      ...
      >
        ...
        <span className="text-sm font-semibold text-gray-800 flex-1">
          Módulo {index + 1} {modulo.nombre.toUpperCase()}
        </span>
        {!isOpen && (
          <span className="text-xs text-gray-400 shrink-0">
            {modulo.submodulos.length} sesión{modulo.submodulos.length !== 1 ? "es" : ""}
          </span>
        )}
        {isOpen && !isLoading && (
          <span className="text-xs text-gray-400 shrink-0">
        ...
        <div className="divide-y divide-gray-100">
          ...
          {!isLoading && archivos.length === 0 && (
            <p className="px-5 py-4 text-sm text-gray-400">
              Sin archivos en este módulo.
            </p>
          )}

          {!isLoading && archivos.map((material) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm flex-1 truncate ${active ? "text-[#0056D2] font-medium" : "text-gray-700"}`}>
...
export default function BienvenidaView({ programaId }: Props) {
  ...
  const { data: materialesData } = useGetMaterialesProgramaQuery(programaId);

  const allMateriales = useMemo(() => materialesData?.results ?? [], [materialesData]);

  // Solo el video de bienvenida: material de tipo video sin módulo
  ...
  const [openSet, setOpenSet] = useState<Set<number>>(() =>
    new Set(modulos[0]?.id !== undefined ? [modulos[0].id] : []),
  );
  const [selectedMaterial, setSelectedMaterial] = useState<Material | null>(null);

  const allExpanded = modulos.length > 0 && openSet.size === modulos.length;

  const toggleExpand = () => {
    setOpenSet(allExpanded ? new Set() : new Set(modulos.map((m) => m.id)));
  };
    ...
    <div className="max-w-3xl mx-auto px-6 py-8 space-y-10">
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-gray-900">Temario</h2>
          {modulos.length > 0 && (
            <button
              type="button"
        ...
        {modulos.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-8">
            Este programa aún no tiene módulos configurados.
          </p>
        ) : (
