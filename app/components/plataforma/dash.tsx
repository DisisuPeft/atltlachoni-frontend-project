...
import { ProgramaDocente } from "@/redux/features/types/control-escolar/type";
...
function AlumnoProgramaCard({ programa }: { programa: Programa }) {
  return (
    <Link
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11px] font-semibold text-[#0056D2] uppercase tracking-wide mb-0.5">{programa.tipo}</p>
        <h3 className="font-semibold text-gray-900 text-sm truncate">{programa.nombre}</h3>
        <p className="text-xs text-gray-400 mt-0.5">{programa.modulos.length} módulos</p>
      </div>
...
function DocenteProgramaCard({ programa }: { programa: ProgramaDocente }) {
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-4 hover:border-purple-200 hover:shadow-md transition-all">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11px] font-semibold text-purple-600 uppercase tracking-wide mb-0.5">{programa.tipo}</p>
        <h3 className="font-semibold text-gray-900 text-sm truncate">{programa.nombre}</h3>
...
export default function Dashboard() {
  ...
  const { data: detail, isLoading: loadingAlumno } = useInscriptionAlumnoDetailQuery(undefined, {
    skip: !user || esDocente,
  });

  const { data: docenteData, isLoading: loadingDocente } = useGetMisProgramasDocenteQuery(undefined, {
    skip: !user || !esDocente,
  });

  const nombre = user?.nombre_completo?.split(" ")[0] ?? "";
  const programasAlumno = detail?.programasInscritos ?? [];
  const programasDocente = docenteData?.programas ?? [];
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 md:py-10 space-y-8">
      ...
      {!esDocente && detail && (
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
            ...
            <p className="text-2xl font-bold text-gray-900">{detail.countCursos}</p>
      ...
      {!esDocente && (
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/plataforma/educacion" className="flex items-center gap-1 text-sm text-[#0056D2] font-medium hover:underline">
              Ver todos <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          <div className="space-y-3">
            {loadingAlumno && <><CardSkeleton /><CardSkeleton /></>}
            {!loadingAlumno && programasAlumno.length === 0 && (
              <div className="bg-white border border-gray-100 rounded-xl p-10 flex flex-col items-center text-center">
                ...
                <p className="font-semibold text-gray-900 mb-1">Sin programas inscritos</p>
                <p className="text-sm text-gray-400">Contacta a tu tutora para inscribirte en un programa.</p>
              </div>
            )}
            {!loadingAlumno && programasAlumno.slice(0, 3).map((p) => (
              <AlumnoProgramaCard key={p.ref} programa={p} />
            ))}
          </div>
        </div>
      )}
      ...
      {esDocente && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-gray-900">Mis programas asignados</h2>
            {docenteData && (
          ...
          <div className="space-y-3">
            ...
            {!loadingDocente && programasDocente.length === 0 && (
              <div className="bg-white border border-gray-100 rounded-xl p-10 flex flex-col items-center text-center">
            ...
            {!loadingDocente && programasDocente.map((p) => (
              <DocenteProgramaCard key={p.ref} programa={p} />
            ))}
          </div>
        </div>
      )}
